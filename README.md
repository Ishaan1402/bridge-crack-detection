...
**Training:** the current pipeline is in [edu/train_v3.ipynb](edu/train_v3.ipynb), training U-Net (SE blocks, bottleneck dropout, deep supervision, AMP/cosine/EMA), and evaluation on the staged test split plus the held-out DeepCrack test. The same scheme can be run outside Colab using `scripts/train.py`.

**Evaluation:** `scripts/verify_metrics.py` reports global and per-image Dice/IoU/Recall/Precision at any set threshold.
...
| -------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| `unet_v3.pth`        | U-Net `[32,64,128,256]` + SE blocks + deep supervision | Trained 2026-08 on multiple data sources, Adam 5e-4 + cosine, EMA.                                          |
...
This test split is the held-out benchmark pulled from multiple data sources for bridge crack images. The old models were trained on UAV-only data which is why they perform poorly here.
...
Data: UAV Kaggle (random 70/15/15 split), DeepCrack train (300), and a subset of a merged 11.2k crack dataset (CRACK500, CFD, GAPS384, Rissbilder, Volker, Sylvie, forest, cracktree200, noncrack), with binary masks and ImageNet normalization.

Model: narrow U-Net + squeeze-and-excitation, bottleneck dropout 0.1, deep supervision, 512px, BCE+Dice, Adam lr 5e-4 with 3-epoch warmup + cosine decay, EMA, AMP, 30 epochs, best-by-val-Dice. Validation Dice ≈ 0.73 global / 0.66 macro at completion.
...
crack-seg/
├── config/          # YAML configs
├── edu/             # Training notebook 
├── scripts/             
│   ├── train.py                # Training script
│   ├── prepare_dataset.py      # Data loading and split train/val/test
│   ├── colab_data.py           # Download helpers for train_v3.ipynb
│   ├── verify_metrics.py       # Checkpoint evaluation 
